...
public class Injector {
	class AgentInitThread extends Thread {
		...
		private void uploadAgent() throws IOException {
			...
			try {
				...
				String cmd = String.format("adb -s %s push %s %s",
						device.getSerialNumber(), LOCAL_AGENT_JAR_LOCATION
				...
				if (logger.isInfoEnabled()) {
					logger.info("uploadAgent() - ADB push exitValue="
							+ exitValue);
				}
	...
	public void injectSwipe(int fromX, int fromY, int toX, int toY) {
		...
		String cmdList = String.format("swipe/%d/%d/%d/%d", fromX, fromY, toX,
				toY);
		injectData(cmdList);
		if (logger.isDebugEnabled()) {
