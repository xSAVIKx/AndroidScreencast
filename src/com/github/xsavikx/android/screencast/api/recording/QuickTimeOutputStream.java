...
public class QuickTimeOutputStream {
  ...
  private abstract class Atom {
     ...
     * 
     * @param type
     *          The type of the atom. A string with a length of 4 characters.
     */
  ...
  public void writeFrame(BufferedImage image, int duration) throws IOException {
    ...
    if (imgWidth == -1) {
      ...
      if (imgWidth != image.getWidth() || imgHeight != image.getHeight()) {
        throw new IllegalArgumentException("Dimensions of image[" + videoFrames.size() + "] (width=" + image.getWidth()
            + ", height=" + image.getHeight() + ") differs from image[0] (width=" + imgWidth + ", height=" + imgHeight);
      }
    }
